...
Windows 也可以双击仓库根目录的 `open-studio.bat`。没有 Python 时下载 Releases 里的这一个 `Studio.exe`，双击即可。打开 `http://127.0.0.1:8787`，默认就是 Z-Image。下拉里还有 FLUX.2 [dev]、Qwen-Image-2512、Krea-2 Turbo、Pixal3D、TripoSplat。密钥写在 `.studio.env`（单文件版在 `%LOCALAPPDATA%\ComfyStudio`），不会进 Git。见文档「Studio」。
