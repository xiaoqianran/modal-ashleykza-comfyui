...
# Closed set. New Studio recipes go in catalog/*.json, not here.
...
LEGACY_PROFILES_DOCS = Path(__file__).resolve().parent / "docs" / "guide" / "_generated_legacy_profiles.md"
...
def profile_comfy_args(name: str) -> tuple[str, ...]:
    return get_profile(name).comfy_args


def profile_node_packs(name: str) -> tuple[str, ...]:
    return get_profile(name).node_packs


def enforce_frozen_profiles() -> None:
    found = frozenset(PROFILES)
    if found != FROZEN_PROFILE_IDS:
        extra = ", ".join(sorted(found - FROZEN_PROFILE_IDS)) or "-"
        missing = ", ".join(sorted(FROZEN_PROFILE_IDS - found)) or "-"
        raise ValueError(
            "recipes.PROFILES is frozen; new Studio recipes use catalog/*.json. "
            f"extra={extra} missing={missing}"
        )


def render_legacy_profiles_table() -> str:
    enforce_frozen_profiles()
    lines = [
        GENERATED_BANNER,
        "",
        "| 名称 | 模型包 | 额外节点 | 说明 |",
    ...
    for name, recipe in PROFILES.items():
        models = ", ".join(f"`{item}`" for item in recipe.model_packs) or "—"
        nodes = ", ".join(f"`{item}`" for item in recipe.node_packs) or "—"
        desc = (recipe.description or "").replace("|", "\\|")
        lines.append(f"| `{name}` | {models} | {nodes} | {desc} |")
    lines.append("")
    return "\n".join(lines)


def write_legacy_profiles_docs(path: Path | None = None) -> Path:
    target = path or LEGACY_PROFILES_DOCS
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(render_legacy_profiles_table(), encoding="utf-8")
    return target
